...
yargs(hideBin(process.argv))
  ...
  .command(
    ...
    (y) =>
      y
        ...
        .option('allow-official', {
          type: 'boolean',
          description: 'Allow posting even if an official responded',
        })
        .option('force-reply-id', {
          type: 'string',
          description: 'Force posting for this response id even if a reply already exists',
        })
        .option('seen-you-message', {
          type: 'string',
          description: 'Custom text for the seen-you notification',
        })
        .option('seen-you-emoji', {
          type: 'string',
          description: 'Emoji to prefix the seen-you notification',
        })
        .option('provider', {
          type: 'string',
          description: 'Override LLM provider (claude|lm_studio|continue|codex)',
        })
        .option('github-only', {
          type: 'boolean',
          description: 'Skip Discord entirely (no token needed)',
        })
        .option('repos', {
          type: 'string',
          array: true,
          description: 'Override repos to sync (e.g. game-ci/unity-builder)',
        })
        .option('repo-dir', {
          type: 'string',
          description: 'Path to local clone of target repo (Claude reads code directly)',
        })
        .option('docs-dir', {
          type: 'string',
          description: 'Path to local clone of documentation repo (skips HTTP docs sync)',
        })
        .option('investigation-issues', {
          type: 'boolean',
          description: 'Create GitHub issues for each investigation in the target repo',
        })
        .option('investigation-repo', {
          type: 'string',
          description: 'Target repo for investigation issues (default: game-ci/help-bot)',
        })
        .option('dispatch-mode', {
          type: 'string',
          choices: ['auto', 'approval', 'countdown', 'triage'] as const,
          description:
            'Dispatch mode: auto (immediate), approval (require reaction), countdown (staged warnings)',
        })
        .option('countdown-hours', {
          type: 'number',
          description: 'Hours between countdown warning stages (default: 24)',
        })
        .option('model', {
          type: 'string',
          description: 'Override LLM model (e.g. claude-opus-4-20250514 for deep investigation)',
        }),
    async (args) => {
      if (!args['github-only']) {
      ...
      await runCycle({
        ...
        dispatchMode: args['dispatch-mode'] as
          | 'auto'
          | 'approval'
          | 'countdown'
          | 'triage'
  ...
  .command(
    ...
    (y) =>
      y
        ...
        .option('investigation-issues', {
          type: 'boolean',
          description: 'Create GitHub issues for each investigation',
        })
        .option('investigation-repo', {
          type: 'string',
          description: 'Target repo for investigation issues',
        })
        .option('dry-run', { type: 'boolean', description: 'Draft responses without posting' }),
    async (args) => {
      if (!args['github-only']) {
  ...
  .command(
    'live',
    'run as a live Discord bot (persistent Gateway connection)',
    (y) =>
      y
        .option('dispatch-mode', {
          type: 'string',
          choices: ['auto', 'approval', 'countdown', 'triage'] as const,
          description: 'Dispatch mode for incoming messages (default: discord_mode from config)',
        })
        .option('repos', {
          type: 'string',
          array: true,
          description: 'GitHub repos for cross-referencing',
        })
        .option('repo-dir', {
          type: 'string',
          description: 'Path to local clone of target repo (Claude reads code directly)',
        ...
        .option('model', {
          type: 'string',
          description: 'Override LLM model (e.g. claude-opus-4-20250514)',
        })
        .option('dry-run', {
          type: 'boolean',
          description: 'Investigate but do not post responses',
        }),
    async (args) => {
      await ensureDiscordToken()
  ...
  .command(
    'sync-data',
    'sync data/ directory to private GitHub repo',
    (y) =>
      y
        .option('remote-repo', {
          type: 'string',
          description: 'Override remote repo (e.g. game-ci/help-bot-data)',
        })
        .option('dry-run', {
          type: 'boolean',
          description: 'Show what would be committed without pushing',
        }),
    async (args) => {
  ...
  .command(
    'nssm <action>',
    'manage the Windows NSSM service',
    (y) =>
      y
        .positional('action', {
        ...
        .option('dispatch-mode', {
          type: 'string',
          description: 'Dispatch mode for live mode (auto, triage, approval)',
        })
    ...
    async (args) => {
      ...
      await manageService(action, {
        ...
        dispatchMode: args['dispatch-mode'] as string | undefined,
        envFile: args['env-file'],
        envVars: args['env-vars'],
      })
    },
  )
  .command(
    'feedback mark-good <responseId>',
    'Mark a response as helpful',
    (y) =>
      y
        .positional('responseId', {
          type: 'string',
          description: 'ID of the response file (without extension)',
        })
        .option('note', {
          type: 'string',
          description: 'Optional note describing why the response was helpful',
        }),
    async (args) => {
  ...
  .command(
    ...
    (y) =>
      y
        ...
        .option('note', {
          type: 'string',
          description: 'Optional note describing why it needs improvement',
        }),
    async (args) => {
      const id = args.responseId
      if (!id) {
  ...
  .command(
    'report summary',
    'Show the most recent cycle statistics',
    () => {},
    async () => {
      await reportSummary()
    },
  ...
  .command(
    'security-scan <repo>',
    'scan synced issues for prompt injection',
    (y) =>
      y.positional('repo', {
        type: 'string',
        description: 'Repo slug (e.g. game-ci-unity-builder)',
      }),
    async (args) => {
      ...
      if (findings.length > 0) {
        ...
        for (const f of findings) {
          console.log(
            `  [${f.severity.toUpperCase()}] ${f.pattern} in ${f.location}${f.lineNumber ? `:${f.lineNumber}` : ''}: ${f.matchedText.substring(0, 80)}`,
          )
        }
      } else {
  ...
  .command(
    'review-quality',
    'interactive quality review of recent responses',
    (y) =>
      y.option('model', {
        type: 'string',
        description: 'Override model for review (e.g. claude-opus-4-20250514)',
      }),
    async (args) => {
  ...
  .command(
    'review-security',
    'interactive security audit of recent responses and system output',
    (y) =>
      y.option('model', {
        type: 'string',
        description: 'Override model for review (e.g. claude-opus-4-20250514)',
