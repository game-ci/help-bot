...
export function buildInvestigationPrompt(options: InvestigationOptions): string {
  ...
  s.push(`- **CLI/API syntax check:** For every command, flag, parameter, or argument in your response, find the exact usage in source code or docs. If you cannot find it, remove it. Never reproduce CLI syntax from memory — it is frequently wrong.`)
  s.push(`- **Quote/escaping check:** Verify that all quoting, spacing, and escaping in code examples exactly matches what the source code expects. A single extra space or wrong quote style can break a command.`)
