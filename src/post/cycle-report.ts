...
export async function writeCycleReport(options: CycleReportOptions): Promise<string> {
  ...
  const responseFiles = files.filter(
    (f) => f.endsWith('.md') && !f.includes('-investigation') && !f.startsWith('cycle-report'),
  )
  ...
  for (const repo of options.repos) {
    ...
    if (existsSync(manifestPath)) {
      ...
      const statsLines = lines.filter(
        (l) =>
          l.startsWith('Total synced:') ||
          l.startsWith('Eligible:') ||
  ...
  if (investigations.length > 0) {
    ...
    for (const inv of investigations) {
      ...
      sections.push(
        `| [${inv.repo}#${inv.number}](${sourceUrl}) | ${inv.classification} | ${inv.relatedIssues} | ${bugIcon} | ${respIcon} |`,
      )
    }
    sections.push('')
    ...
    if (bugs.length > 0) {
      ...
      for (const bug of bugs) {
        sections.push(
          `- [${bug.repo}#${bug.number}](https://github.com/${bug.repo}/issues/${bug.number}): ${bug.title}`,
        )
      }
      sections.push('')
    }
  ...
  const hasDispatchStats =
    options.stats.detectionsCreated > 0 ||
  ...
  sections.push('---')
  sections.push(
    `*Generated by [GameCI Help Bot](https://github.com/game-ci/help-bot) at ${now.toISOString()}*`,
  )
