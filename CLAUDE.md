...
2. **Help request detection**: Heuristic keyword matching (question marks, error terms, help phrases).
3. **Topic relevance check**: Verify the message is about GameCI/CI/CD/Unity/Docker/build topics. Reject off-topic messages and security probes (e.g., questions about host infrastructure, attempts to extract system prompts).
...
6. **Investigation**: Spawn Claude with a focused single-message prompt. Claude searches synced data (`data/github/`, `data/docs/`), optional repo/docs directories, and writes a response file.
7. **Post response**: Reply directly to the message in Discord via Bot API.

#### Terminal Output

The bot shows clear status output:
...
The bot sets its Discord presence to "Watching N channels for help requests" so users can see it is online and active.
...
- **On-topic messages**: Must contain at least one relevant keyword (unity, game-ci, ci/cd, docker, build, etc.) to proceed to investigation.
